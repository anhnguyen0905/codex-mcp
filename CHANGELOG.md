...
- **First three Step-7d authored skills** — `performance-marketing`, `unit-economics`, and `media-planning` were written into the skill library through the Step 7 acquire-or-author path (gap analysis → no adoptable skill found → author with provenance labels → reindex). The 100-request scope run went 62/100 → **70/100**; the 32-scenario eval stayed 32/32. The skills live in the operator's `~/claude-skill-library`, not in this repo.
